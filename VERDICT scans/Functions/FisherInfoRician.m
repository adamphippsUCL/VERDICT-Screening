...
%         disp(paramSetIndx)
    ...
    if params(paramIndx) == 0
        ...
        % Add Rician noise average
        signalUp = RicianNoiseAverage(signalUp, opts.NoiseSigma); 

        paramsDown = params;
        signalDown = simulateSignal(paramsDown, scan_params, opts.modeltype);
        % Add Rician noise average
        signalDown = RicianNoiseAverage(signalDown, opts.NoiseSigma);
    ...
    elseif params(paramIndx) == max(paramGridFlat(:, paramIndx))
        ...
        partialS = ( signalUp - signalDown )/(2*0.001);
    end
